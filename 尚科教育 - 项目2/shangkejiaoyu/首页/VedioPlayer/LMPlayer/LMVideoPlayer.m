...
- (void)setPlayerModel:(LMPlayerModel *)playerModel {
    ...
    [self.videoPlayerView.coverControlView syncCoverImageViewWithTitleString:self.playerModel.title];
...
- (void)autoPlayTheVideo {
    NSString *user = [[NSUserDefaults standardUserDefaults] objectForKey:@"userID"];
        NSString *priceStr = [[NSUserDefaults standardUserDefaults] objectForKey:@"priceStatus"];
    ...
    if ([priceStr isEqualToString:@"0.00"]) {
        ...
        if ([user isEqualToString:@"1"]) {
            ...
            if (vedioID == nil) {
                [MBProgressHUD showMessage:@"请先购买该视频！" toView:self.videoPlayerView delay:1.0];
                return;
            }else {
...
/** 倍速按钮被点击 */
// 竖屏下
- (void)portraitRateButtonClickWithTag:(NSInteger)tag {

    // 延迟隐藏控制层
    [self.videoPlayerView.playerControlView autoFadeOutControlView];
    NSLog(@"点击倍速切换了 %ld",(long)tag);
    if (tag == 1000) {
        self.playerMgr.player.playbackRate  = 1.0 ;
        [MBProgressHUD showMessage:@"已切换至1.0倍速" toView:self.videoPlayerView delay:1.5];

    }else if (tag == 1001){
        self.playerMgr.player.playbackRate  = 1.0 * 1.25;
        [MBProgressHUD showMessage:@"已切换至1.25倍速" toView:self.videoPlayerView delay:1.5];

    }else if (tag == 1002){
        self.playerMgr.player.playbackRate  = 1.0 * 1.5;
        [MBProgressHUD showMessage:@"已切换至1.5倍速" toView:self.videoPlayerView delay:1.5];

    }else if (tag == 1003){
        self.playerMgr.player.playbackRate  = 1.0 * 2;
        [MBProgressHUD showMessage:@"已切换至2.0倍速" toView:self.videoPlayerView delay:1.5];

    }

}
//横屏下（全屏下）
- (void)landRateButtonClickWithTag:(NSInteger)tag {
...
//- (void)loadingViewRateButtonClick {
//    if ([self.delegate respondsToSelector:@selector(playerRateButtonClick:)]) {
//        [self.delegate playerRateButtonClick];
//    }
//}
/** 分享按钮被点击 */
