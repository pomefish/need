...
//#import "landRateView.h"
#import "rateView.h"
@interface LMLandScapeControlView ()<PlayRateButtonDelegate>
/** 顶部工具栏 */
...
- (void)playButtonDelegateWithTag:(NSInteger)tag{
    ...
    if ([self.delegate respondsToSelector:@selector(landRateButtonClickWithTag:)]) {
        [self.delegate landRateButtonClickWithTag:tag];
    }
}
...
- (rateView *)rateView{
    if (!_rateView) {
        ...
        _rateView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.3];
