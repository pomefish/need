            ...
            if ([pay_status isEqualToString:@"2"]) {
                ...
                [[AFNetworkReachabilityManager sharedManager ] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
                    switch (status) {
                        ...
                        case 1: {
                            NSLog(@"GPRS网络");
                            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"您正在使用非WiFi网络，播放将产生流量费用。" message:nil delegate:weakSelf cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
                ...
                [MBProgressHUD showMessage:@"请先购买该视频！" toView:self delay:1.0];
            }
        }
    }];
...
//    UIInterfaceOrientationLandscapeRight     = UIDeviceOrientationLandscapeLeft
    ...
    if (device.orientation == UIInterfaceOrientationLandscapeRight || device.orientation == UIInterfaceOrientationLandscapeLeft ) {
...
{
    if (!_downView) {
        _downView =[[UIView alloc]initWithFrame:CGRectMake(0, viewHeight, viewWidth, LXCommonViewH)];
...
{
    if (!_backBtn) {
        _backBtn =[LxButton LXButtonWithTitle:@"" titleFont:Font(13.0) Image:nil backgroundImage:nil backgroundColor:[UIColor clearColor] titleColor:[UIColor clearColor] frame:CGRectMake(0, 10, 100, 20)];
...
{
    if (!_titleLabel) {
        _titleLabel =[UILabel LXLabelWithText:self.videoTitle textColor:[UIColor whiteColor] backgroundColor:[UIColor clearColor] frame:CGRectMake(10, (LXCommonViewH - 40)/2, viewWidth -200, 40) font:Font(13.0) textAlignment:NSTextAlignmentCenter];
    }
    return _titleLabel;
}
-(LxButton *)startBtn
{
    if (!_startBtn) {
        ...
        _startBtn =[LxButton LXButtonWithTitle:nil titleFont:nil Image:[UIImage imageNamed:@"playBtn"] backgroundImage:nil backgroundColor:nil titleColor:nil frame:CGRectMake(10, (LXCommonViewH - 30)/2, 30, 30)];
...
-(UILabel *)currentTimeLabel
{
    if (!_currentTimeLabel) {
        ...
        _currentTimeLabel = [UILabel LXLabelWithText:@"00:00" textColor: [UIColor colorWithRed:220 / 255.0 green:220 / 255.0 blue:220 / 255.0 alpha:1.0] backgroundColor:nil frame:CGRectMake(timeX, timeY, 40, 20) font:Font(13.0) textAlignment:NSTextAlignmentRight];
...
-(UILabel *)gangLabel
{
    if (!_gangLabel) {
        ...
        _gangLabel = [UILabel LXLabelWithText:@"/" textColor: [UIColor colorWithRed:220 / 255.0 green:220 / 255.0 blue:220 / 255.0 alpha:1.0] backgroundColor:nil frame:CGRectMake(CGRectGetMaxX(self.currentTimeLabel.frame), timeY, 10, 20) font:Font(13.0) textAlignment:NSTextAlignmentCenter];
...
{
    if (!_sumTimeLabel) {
        _sumTimeLabel =[UILabel LXLabelWithText:@"00:00" textColor: [UIColor colorWithRed:220 / 255.0 green:220 / 255.0 blue:220 / 255.0 alpha:1.0] backgroundColor:nil frame:CGRectMake(CGRectGetMaxX(self.gangLabel.frame), timeY, 40, 20) font:Font(13.0) textAlignment:NSTextAlignmentLeft];
...
{
    if (!_fullScreenBtn) {
        _fullScreenBtn =[LxButton LXButtonWithTitle:nil titleFont:nil Image:[UIImage imageNamed:@"全屏"] backgroundImage:nil backgroundColor:nil titleColor:nil frame:CGRectMake(CGRectGetMaxX(self.sumTimeLabel.frame) , 14, 50, 50)];
...
//                    设置横屏
                ...
                if ([[UIDevice currentDevice] respondsToSelector:@selector(setOrientation:)]) {
                    ...
                    //设置竖屏
                    weakSelf.frame = CGRectMake(0, 64, kScreenWidth, kScreenWidth *9/16);
                    viewWidth = kScreenWidth;
                    viewHeight = kScreenWidth *9/16;
...
//       _animationView.animationImages = Images;
        [_animationView sd_setImageWithURL:[NSURL URLWithString:_imgaeUrl] placeholderImage:[UIImage imageNamed:@"banner1"]];
...
//    _backBtn.frame= CGRectMake(0, 10, 80, 40);
    _titleLabel.frame= CGRectMake(5, (LXCommonViewH - 40)/2, viewWidth -200, 40) ;
    _progressContainer .frame =CGRectMake(CGRectGetMaxX(self.startBtn.frame)+10, (LXCommonViewH - 40)/2, viewWidth - CGRectGetMaxX(self.startBtn.frame) - 140, 40);
    ...
    timeX = CGRectGetMaxX(self.progressContainer.frame)+5;
    timeY =  (LXCommonViewH - 20)/2;
    _currentTimeLabel .frame   = CGRectMake(timeX - 20, timeY, 60, 20) ;
    ...
    self.volume.transform = CGAffineTransformMakeRotation(M_PI * 1.5);
    self.volume.center = CGPointMake(self.startBtn.center.x, viewHeight / 2);
    _fullScreenBtn . frame =  CGRectMake(CGRectGetMaxX(self.sumTimeLabel.frame) - 20, (LXCommonViewH - 30)/2, 30, 30);
    _animationView .frame = CGRectMake(0, 0, viewWidth, viewHeight);
