...
//            hotView.backgroundColor = [UIColor lightGrayColor];
...
- (void)layoutSubviews
{
    ...
    UITapGestureRecognizer *tapgesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(touchIndex)];
...
- (void)touchIndex{
    ...
    if ([self.delegate respondsToSelector:@selector(selectBtnWithIndex:)]) {
        [self.delegate selectBtnWithIndex:currINdex];
    }
}
