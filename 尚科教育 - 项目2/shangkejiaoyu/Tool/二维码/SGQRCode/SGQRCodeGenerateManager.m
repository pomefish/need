...
+ (UIImage *)generateWithLogoQRCodeData:(NSString *)data logoImageName:(NSString *)logoImageName logoScaleToSuperView:(CGFloat)logoScaleToSuperView {
    ...
    if([icon_imageName isEqualToString:@"http://shangke.mingtaokeji.com/"]){
        icon_image = [UIImage imageNamed:@"personalLogo"];
    }else{
        NSRange  urlRange = [icon_imageName rangeOfString:@"://"];
        if (urlRange.location == NSNotFound) {
            icon_image = [UIImage imageNamed:@"personalLogo"];
        }
        else{
            icon_image = [UIImage imageWithData: [NSData dataWithContentsOfURL:[NSURL URLWithString:icon_imageName]]];
        }
    }
