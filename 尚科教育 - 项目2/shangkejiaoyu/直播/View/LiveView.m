...
@interface LiveView ()<UIScrollViewDelegate>
...
- (instancetype)initWithFrame:(CGRect)frame SegmentViewHeight:(CGFloat)segmentViewHeight titleArray:(NSArray *)titleArray Controller:(UIViewController *)controller lineWidth:(float)lineW lineHeight:(float)lineH buttonY:(float)buttonY{
    ...
    if (self = [super initWithFrame:frame]){
        ...
        for (int i = 0; i < self.nameArray.count; i++) {
            ...
            button.tag = i;
            button.frame = CGRectMake(i*(frame.size.width/self.nameArray.count), buttonY, frame.size.width/self.nameArray.count, segmentViewHeight);
        ...
        self.indicateView = [[UILabel alloc]initWithFrame:CGRectMake((avgWidth - lineW),segmentViewHeight-lineH - 5, 20, lineH)];
