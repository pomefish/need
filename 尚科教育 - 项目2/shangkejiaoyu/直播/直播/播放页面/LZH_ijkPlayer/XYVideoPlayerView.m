...
//    NSString *type = [HttpRequest checkNetworkStatus];
...
//        [self.player pause];
        ...
        switch (status) {
            case -1:
                NSLog(@"未知网络");
                [AlertView(@"温馨提示", @"无法连接网络，请稍后再试！", @"取消", @"确定") show];
                break;
            case 0:
                NSLog(@"网络不可达");
                [AlertView(@"温馨提示", @"无法连接网络，请稍后再试！", @"取消", @"确定") show];
            ...
            case 1:
                NSLog(@"GPRS网络");
                [AlertView(@"温馨提示", @"非WiFi环境下是否继续观看视频？", @"取消", @"确定") show];
                break;
            case 2:
                NSLog(@"wifi网络");
...
//        make.edges.equalTo(self.sliderBackView);
    ...
    [self.playOrPauseBtn setImage:[UIImage imageNamed:@"playBtn"] forState:UIControlStateNormal];
...
//    [self.fullScreenBtn setImage:[UIImage imageNamed:@"btn_全屏"] forState:UIControlStateNormal];
//    [self.fullScreenBtn setImage:[UIImage imageNamed:@"btn_缩屏"] forState:UIControlStateSelected];
    //系统的音量
...
- (void)moviePlayBackDidFinish:(NSNotification*)notification
{
    ...
    switch (reason)
    {
        case IJKMPMovieFinishReasonPlaybackEnded:
            NSLog(@"--播发完毕---playbackStateDidChange: IJKMPMovieFinishReasonPlaybackEnded: %d\n", reason);
